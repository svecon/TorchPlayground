-- Modified from gnuplot package https://github.com/torch/gnuplot
-- to add a command which builds plot that shows a splot and one or
-- more optimization paths taken by various methods
...
local function getgnuplotdefaultterm(os)
   ...
   elseif os == 'mac' and gnuplothasterm('x11') then
      ...
      print('Can not find any of the default terminals for ' .. os .. '. ' ..
            'You can manually set terminal by gnuplotx.setterm("terminal-name")')
      return nil
   end
end
...
function gnuplotx.setgnuplotexe(exe)
   ...
   if term == nil then
      print('You have manually set the gnuplot exe and I can not find default terminals, run gnuplotx.setterm("terminal-name") to set term type')
   end
end

function gnuplotx.setterm(term)
...
function gnuplotx.figprint(fname)
...
function gnuplotx.figure(n, config)
   config = config or {}
   local gp = getfigure(n)
   writeToCurrent('set term ' .. _gptable[_gptable.current].term .. ' ' .. _gptable.current .. '\n')
...
function gnuplotx.xlabel(label)
   if not _gptable.hasrefresh then
      print('gnuplotx.xlabel disabled')
      return
   end
   writeToCurrent('set xlabel "' .. label .. '"')
   refreshCurrent()
end
function gnuplotx.ylabel(label)
   if not _gptable.hasrefresh then
      print('gnuplotx.ylabel disabled')
      return
   end
   writeToCurrent('set ylabel "' .. label .. '"')
   refreshCurrent()
end
function gnuplotx.zlabel(label)
   if not _gptable.hasrefresh then
      print('gnuplotx.zlabel disabled')
      return
   end
   writeToCurrent('set zlabel "' .. label .. '"')
   refreshCurrent()
end
function gnuplotx.title(label)
   if not _gptable.hasrefresh then
      print('gnuplotx.title disabled')
      return
   end
   writeToCurrent('set title "' .. label .. '"')
   refreshCurrent()
end
function gnuplotx.grid(toggle)
   if not _gptable.hasrefresh then
      print('gnuplotx.grid disabled')
      return
   end
   if toggle then
...
function gnuplotx.movelegend(hloc,vloc)
   if not _gptable.hasrefresh then
      print('gnuplotx.movelegend disabled')
      return
   end
...
function gnuplotx.axis(axis)
   if not _gptable.hasrefresh then
      print('gnuplotx.axis disabled')
      return
   end
   if axis == 'auto' then
...
-- displaypath({x2,y2,z2}, {'name1',pt_list_3d}, {'name2',pt_list_3d} ...)
function gnuplotx.displaypaths(...)
   local arg = {...}
   if select('#', ...) == 0 then
      error('no inputs, expecting at least a matrix')
   end
   ...
   if type(arg[1]) ~= "table" then
      error('first argument must be a table {xx,yy,zz} with 2D x y and z coordinates of a surface.')
   end
   local _, xx, yy, zz = getsplotvars(arg[1])

   for i=2,#arg do
     legends[#legends + 1] = arg[i][1]
     paths[#paths + 1] = arg[i][2]
   end

   -- prepare the splot part of the plot (error surface plotting)
   local _, splot_data = gnu_splot_string({'error'}, {xx}, {yy}, {zz})

   
   local plotstr = string.format('%s\n','set contour base')
   plotstr = string.format('%s%s\n',plotstr,'set cntrparam bspline\n')
   --plotstr = string.format('%s%s\n',plotstr,'set cntrparam levels auto\n')
   --plotstr = string.format('%s%s\n',plotstr,'set style data lines\n')
   plotstr = string.format('%s%s\n',plotstr,'set hidden3d\n')


   -- plotstr
   plotstr = plotstr .. "splot '-' title 'error' with lines"

   for i=1,#legends do
     plotstr = plotstr .. ", '-' title '" .. legends[i] .. "' with linespoints"
   end

   writeToCurrent(plotstr)
   writeToCurrent(splot_data)

   -- prepare the path part of the plot
   for i=1,#legends do
     local _, paths_data = gnu_scatter3_string({legends[i]}, {paths[i][{{},1}]}, {paths[i][{{},2}]}, {paths[i][{{},3}]})
     writeToCurrent(paths_data)
  end
...
function gnuplotx.hist(tensor,bins,min,max)
   local h = gnuplotx.histc(tensor,bins,min,max)
   local x_axis = torch.Tensor(#h)
   for i = 1,#h do
      x_axis[i] = h[i].val
   end
   gnuplotx.bar(x_axis, h.raw)
   return h
end
